    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li>
                    <a href="<?= url('domain-name/' . $data->domain_name->domain_name_id) ?>"><?= l('domain_name.breadcrumb') ?></a><i class="fas fa-fw fa-angle-right"></i>
                </li>
                <li class="active" aria-current="page"><?= l('domain_name_update.breadcrumb') ?></li>
            </ol>
        </nav>
    <?php endif ?>

    <div class="d-flex justify-content-between align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0"><i class="fas fa-fw fa-xs fa-network-wired mr-1"></i> <?= sprintf(l('global.update_x'), $data->domain_name->name) ?></h1>
        ...
        <div class="card-body">
            ...
            <form action="" method="post" role="form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control <?= \Altum\Alerts::has_field_errors('name') ? 'is-invalid' : null ?>" value="<?= $data->domain_name->name ?>" required="required" />
                    <?= \Altum\Alerts::output_field_error('name') ?>
                </div>

                <div class="form-group">
                    <label for="target"><i class="fas fa-fw fa-sm fa-network-wired text-muted mr-1"></i> <?= l('domain_name.input.target') ?></label>
                    <input type="text" id="target" name="target" class="form-control <?= \Altum\Alerts::has_field_errors('target') ? 'is-invalid' : null ?>" value="<?= $data->domain_name->target ?>" placeholder="<?= l('domain_name.input.target_placeholder') ?>" required="required" />
                        ...
                        <?php foreach($data->notification_handlers as $notification_handler): ?>
                            <div class="col-12 col-lg-6">
                                <div class="custom-control custom-checkbox my-2">
                                    <input id="whois_notifications_<?= $notification_handler->notification_handler_id ?>" name="whois_notifications[]" value="<?= $notification_handler->notification_handler_id ?>" type="checkbox" class="custom-control-input" <?= in_array($notification_handler->notification_handler_id, $data->domain_name->whois_notifications->whois_notifications ?? []) ? 'checked="checked"' : null ?>>
                ...
                <div class="form-group">
                    ...
                    <select id="whois_notifications_timing" name="whois_notifications_timing" class="custom-select" required="required">
                        <?php foreach($data->domain_name_timings as $key => $value): ?>
                            <option value="<?= $key ?>" <?= $data->domain_name->whois_notifications->whois_notifications_timing == $key ? 'selected="selected"' : null ?>><?= sprintf(l('domain_name.input.whois_notifications_timing_input'), $value) ?></option>
                        <?php endforeach ?>
                    </select>
                </div>

                <div class="form-group custom-control custom-switch">
                    <input id="is_enabled" name="is_enabled" type="checkbox" class="custom-control-input" <?= $data->domain_name->is_enabled ? 'checked="checked"' : null?>>
                    <label class="custom-control-label" for="is_enabled"><?= l('domain_name.input.is_enabled') ?></label>
                </div>
                ...
                <div class="collapse" id="advanced_container">
                    <div class="form-group">
                        ...
                        <input type="number" min="0" max="100000" id="ssl_port" name="ssl_port" class="form-control <?= \Altum\Alerts::has_field_errors('ssl_port') ? 'is-invalid' : null ?>" value="<?= $data->domain_name->ssl_port ?? 443 ?>" required="required" />
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            <?php foreach($data->notification_handlers as $notification_handler): ?>
                                <div class="col-12 col-lg-6">
                                    <div class="custom-control custom-checkbox my-2">
                                        <input id="ssl_notifications_<?= $notification_handler->notification_handler_id ?>" name="ssl_notifications[]" value="<?= $notification_handler->notification_handler_id ?>" type="checkbox" class="custom-control-input" <?= in_array($notification_handler->notification_handler_id, $data->domain_name->ssl_notifications->ssl_notifications ?? []) ? 'checked="checked"' : null ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select id="ssl_notifications_timing" name="ssl_notifications_timing" class="custom-select" required="required">
                            <?php foreach($data->domain_name_timings as $key => $value): ?>
                                <option value="<?= $key ?>" <?= $data->domain_name->ssl_notifications->ssl_notifications_timing == $key ? 'selected="selected"' : null ?>><?= sprintf(l('domain_name.input.ssl_notifications_timing_input'), $value) ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="project_id" name="project_id" class="custom-select">
                                ...
                                <?php foreach($data->projects as $project_id => $project): ?>
                                    <option value="<?= $project_id ?>" <?= $data->domain_name->project_id == $project_id ? 'selected="selected"' : null ?>><?= $project->name ?></option>
                                <?php endforeach ?>
                            </select>
