...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-network-wired text-primary-900 mr-2"></i> <?= l('admin_domain_names.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/domain-names?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/domain-names?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="custom-select custom-select-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_domain_names.table.name') ?></option>
                                <option value="target" <?= $data->filters->search_by == 'target' ? 'selected="selected"' : null ?>><?= l('admin_domain_names.table.target') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/domain-names/bulk' ?>" method="post" role="form">
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.user') ?></th>
                <th><?= l('admin_domain_names.table.domain_name') ?></th>
            ...
            <?php foreach($data->domain_names as $row): ?>
                <?php //ALTUMCODE:DEMO if(DEMO) {$row->user_email = 'hidden@demo.com'; $row->user_name = $row->target = 'hidden on demo';} ?>
                <tr>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex flex-column">
                            ...
                            <div class="text-muted">
                                <img referrerpolicy="no-referrer" src="<?= get_favicon_url_from_domain($row->target) ?>" class="img-fluid icon-favicon mr-1" loading="lazy" />
                                <span><?= $row->target ?></span>
                                <a href="<?= 'https://' . $row->target ?>" target="_blank" rel="noreferrer"><i class="fas fa-fw fa-xs fa-external-link-alt ml-1"></i></a>
                            </div>
                        </div>
                    </td>
                    <td class="text-nowrap">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/domain-names/admin_domain_name_dropdown_button.php', ['id' => $row->domain_name_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'path' => 'admin/domain-names/delete/'
]), 'modals'); ?>
