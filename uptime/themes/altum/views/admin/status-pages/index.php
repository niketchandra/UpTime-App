...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-wifi text-primary-900 mr-2"></i> <?= l('admin_status_pages.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/status-pages?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/status-pages?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="custom-select custom-select-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_status_pages.table.name') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/status-pages/bulk' ?>" method="post" role="form">
    ...
    <table class="table table-custom">
        ...
        <tr>
            ...
            <th><?= l('global.user') ?></th>
            <th><?= l('admin_status_pages.table.name') ?></th>
            <th><?= l('admin_status_pages.table.pageviews') ?></th>
        ...
        <?php foreach($data->status_pages as $row): ?>
            <tr>
                ...
                <td class="text-nowrap">
                    <?= $row->name ?><a href="<?= url('status-page-redirect/' . $row->status_page_id) ?>" rel="noreferrer" target="_blank"><i class="fas fa-fw fa-xs fa-external-link-alt text-muted ml-1"></i></a>
                </td>
                <td class="text-nowrap">
                ...
                <td>
                    <div class="d-flex justify-content-end">
                        <?= include_view(THEME_PATH . 'views/admin/status-pages/admin_status_page_dropdown_button.php', ['id' => $row->status_page_id, 'resource_name' => $row->name]) ?>
                    </div>
                </td>
            </tr>
        <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'path' => 'admin/status-pages/delete/'
]), 'modals'); ?>
