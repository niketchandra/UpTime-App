...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-plug text-primary-900 mr-2"></i> <?= l('admin_dns_monitors.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/dns-monitors?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/dns-monitors?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
...
<form id="table" action="<?= SITE_URL . 'admin/dns-monitors/bulk' ?>" method="post" role="form">
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.user') ?></th>
                <th><?= l('admin_dns_monitors.table.monitor') ?></th>
                <th><?= l('admin_dns_monitors.table.stats') ?></th>
            ...
            <?php foreach($data->dns_monitors as $row): ?>
                ...
                <tr>
                    <td data-bulk-table class="d-none">
                        <div class="custom-control custom-checkbox">
                            <input id="selected_monitor_id_<?= $row->dns_monitor_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->dns_monitor_id ?>" />
                            <label class="custom-control-label" for="selected_monitor_id_<?= $row->dns_monitor_id ?>"></label>
                        </div>
                    </td>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex flex-column">
                            <span class="text-muted"><?= sprintf(l('admin_dns_monitors.table.total_checks'), nr($row->total_checks)) ?></span>
                            <span class="text-muted"><?= sprintf(l('admin_dns_monitors.table.total_changes'), nr($row->total_changes)) ?></span>
                        </div>
                    </td>
                    <td class="text-nowrap">
                    ...
                    <td class="text-nowrap">
                        <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('dns_monitor.last_check_datetime') . '<br />' . \Altum\Date::get($row->last_check_datetime, 2) . '<br /><small>' . \Altum\Date::get($row->last_check_datetime, 3) . '</small>' . '<br /><small>(' . \Altum\Date::get_timeago($row->last_check_datetime) . ')</small>' ?>">
                            <i class="fas fa-fw fa-calendar-check text-muted"></i>
                        </span>

                        <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('dns_monitor.last_change_datetime') . '<br />' . \Altum\Date::get($row->last_change_datetime, 2) . '<br /><small>' . \Altum\Date::get($row->last_change_datetime, 3) . '</small>' . '<br /><small>(' . \Altum\Date::get_timeago($row->last_change_datetime) . ')</small>' ?>">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/dns-monitors/admin_dns_monitor_dropdown_button.php', ['id' => $row->dns_monitor_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'path' => 'admin/dns-monitors/delete/'
]), 'modals'); ?>
