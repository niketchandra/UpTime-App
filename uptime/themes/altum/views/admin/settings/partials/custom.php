...
<div>
    ...
    <div class="form-group">
        <label for="head_js_status_page"><i class="fab fa-fw fa-sm fa-js text-muted mr-1"></i> <?= l('admin_settings.custom.head_js_status_page') ?></label>
        <textarea id="head_js_status_page" name="head_js_status_page" class="form-control" data-code-editor data-mode="htmlmixed"><?= settings()->custom->head_js_status_page ?></textarea>
        <small class="form-text text-muted"><?= sprintf(l('global.variables'), '<code>' . implode('</code> , <code>',  ['{{WEBSITE_TITLE}}', '{{USER:NAME}}', '{{USER:EMAIL}}', '{{USER:CONTINENT_NAME}}', '{{USER:COUNTRY_NAME}}', '{{USER:CITY_NAME}}', '{{USER:DEVICE_TYPE}}', '{{USER:OS_NAME}}', '{{USER:BROWSER_NAME}}', '{{USER:BROWSER_LANGUAGE}}', '{{USER:USER_ID}}', '{{USER:PLAN_ID}}']) . '</code>') ?></small>
    </div>

    <div class="form-group">
        <label for="head_css_status_page"><i class="fab fa-fw fa-sm fa-css3 text-muted mr-1"></i> <?= l('admin_settings.custom.head_css_status_page') ?></label>
        <textarea id="head_css_status_page" name="head_css_status_page" class="form-control" data-code-editor data-mode="css"><?= settings()->custom->head_css_status_page ?></textarea>
    </div>

    <div class="form-group">
        <label for="body_content_status_page"><i class="fab fa-fw fa-sm fa-html5 text-muted mr-1"></i> <?= l('admin_settings.custom.body_content_status_page') ?></label>
        <textarea id="body_content_status_page" name="body_content_status_page" class="form-control" data-code-editor data-mode="htmlmixed"><?= settings()->custom->body_content_status_page ?></textarea>
        <small class="form-text text-muted"><?= l('admin_settings.custom.body_content_help') ?></small>
    </div>
</div>
