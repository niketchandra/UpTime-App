...
<div class="col-12 col-md-6 col-lg-4 p-3 position-relative" data-tool-id="<?= $data->tool_id ?>" data-tool-name="<?= l('tools.' . $data->tool_id . '.name') ?>">
    <div class="card d-flex flex-column h-100 overflow-hidden">
        <div class="card-body text-truncate">
            ...
            <a href="<?= url('tools/' . str_replace('_', '-', $data->tool_id)) ?>" class="stretched-link text-decoration-none text-dark">
                <strong><?= l('tools.' . $data->tool_id . '.name') ?></strong>
            </a>
            <p class="text-truncate text-muted small m-0"><?= l('tools.' . $data->tool_id . '.description') ?></p>
