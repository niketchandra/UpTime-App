...
class Pay extends Controller {
        ...
        switch($_POST['payment_type']) {
            ...
            case 'recurring':
                ...
                foreach($response->body->data as $plan) {
                    if($plan->amount == $price && $plan->interval == $flutterwave_payment_frequency) {
                        $payment_plan = $plan;
                    }
                }
    ...
    private function lemonsqueezy() {
        ...
        /* Final price */
        $formatted_price = in_array(currency(), ['MGA', 'BIF', 'CLP', 'PYG', 'DJF', 'RWF', 'GNF', 'UGX', 'JPY', 'VND', 'VUV', 'XAF', 'KMF', 'KRW', 'XOF', 'XPF']) ? number_format($price, 0, '.', '') : number_format($price, 2, '.', '') * 100;

        $price = number_format($price, 2, '.', '');

        /* Generate the payment link */
        $response = \Unirest\Request::post(Lemonsqueezy::$api_url . 'checkouts', Lemonsqueezy::get_headers(), \Unirest\Request\Body::json([
            'data' => [
                'type' => 'checkouts',

                'attributes' => [
                    'custom_price' => $formatted_price,
                    'expires_at' => (new \DateTime('now', new \DateTimeZone('UTC')))->add(new \DateInterval('P1D'))->format(\DateTime::ATOM),
                    'checkout_data' => [
                        'email' => $this->user->email,
                        'name' => $this->user->name,
                        'custom' => [
        ...
        if($response->code === 201) {
            if(!empty($response->body->data->attributes->url)) {
                /* Redirect to payment */
                header('Location: ' . $response->body->data->attributes->url); die();
            } else {
                Alerts::add_error(l('pay.error_message.failed_payment'));
                redirect('pay/' . $this->plan_id . '?' . (isset($_GET['trial_skip']) ? '&trial_skip=true' : null) . (isset($_GET['code']) ? '&code=' . $_GET['code'] : null));
            }
    ...
    private function myfatoorah() {
        ...
        try {
            $response = \Unirest\Request::post(
                ...
                \Unirest\Request\Body::json([
                    'InvoiceValue' => intval($price),
                    'CustomerName' => $this->user->name,
                    'CustomerEmail' => $this->user->email,
                    'WebhookUrl'  => SITE_URL . 'webhook-myfatoorah',
                    'NotificationOption' => 'LNK',
                    'CustomerReference' => $custom_id,
                    'CallBackUrl' => url('pay/' . $this->plan_id . $this->return_url_parameters('success', $base_amount, $price, $code, $discount_amount)),
                    'ErrorUrl' => url('pay/' . $this->plan_id . $this->return_url_parameters('cancel', $base_amount, $price, $code, $discount_amount)),
                ])
            );
        } catch (\Exception $exception) {
            if(DEBUG || \Altum\Authentication::is_admin()) {
        ...
        if($response->code === 200) {
            if(!empty($response->body->Data->InvoiceURL)) {
                /* Redirect to payment */
                header('Location: ' . $response->body->Data->InvoiceURL); die();
            } else {
                Alerts::add_error(l('pay.error_message.failed_payment'));
                redirect('pay/' . $this->plan_id . '?' . (isset($_GET['trial_skip']) ? '&trial_skip=true' : null) . (isset($_GET['code']) ? '&code=' . $_GET['code'] : null));
            }
