...
class DomainUpdate extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(in_array($_POST['host'], settings()->status_pages->blacklisted_domains)) {
                Alerts::add_field_error('host', l('status_page.error_message.blacklisted_domain'));
            }

            if(!empty($_POST['custom_index_url']) && in_array(get_domain_from_url($_POST['custom_index_url']), settings()->status_pages->blacklisted_domains)) {
                Alerts::add_field_error('custom_index_url', l('status_page.error_message.blacklisted_domain'));
            }

            if(!empty($_POST['custom_not_found_url']) && in_array(get_domain_from_url($_POST['custom_not_found_url']), settings()->status_pages->blacklisted_domains)) {
                Alerts::add_field_error('custom_not_found_url', l('status_page.error_message.blacklisted_domain'));
            }
