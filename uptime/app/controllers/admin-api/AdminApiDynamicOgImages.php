...
class AdminApiDynamicOgImages extends Controller {
    ...
    private function post() {
            ...
            try {
                ...
                if(!empty($already_existing_file_name)) {
                    $s3->deleteObject([
                        ...
                        'Key' => UPLOADS_URL_PATH . Uploads::get_path('dynamic_og_images') . $already_existing_file_name,
                    ]);
                }
        ...
        if(!empty($already_existing_file_name) && file_exists(UPLOADS_PATH . Uploads::get_path('dynamic_og_images') . $already_existing_file_name)) {
            unlink(UPLOADS_PATH . Uploads::get_path('dynamic_og_images') . $already_existing_file_name);
        }
